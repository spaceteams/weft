...
export function explainDiffs(model: CompiledModel, result: OverlayEvaluationResult, deltas: readonly ValueDelta[]): readonly ExplainedDelta[] {
...
export function diffToAsciiTree(deltas: readonly ExplainedDelta[], options?: DiffTreeOptions): string {
  if (deltas.length === 0) {
    return ""
  }
